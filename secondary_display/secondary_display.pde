...
void drawTitle() { 
  ...
  textAlign(CENTER);
  textSize(12);
  text("phase="+delta%360,centX+130,centY+120);
  textSize(14);
  text("x="+x,centX-620,height-20);
  text("y="+y,centX-300,height-20);
  text("voltage="+value1+"mV",centX,height-20);
  text("gamma="+gamma,centX+300,height-20);
  text("ambient sensor ver1.0.1",centX+620,height-20);
  text("average="+ave,centX-600,20);
  text("current voltage",centX-70,20);
  text("recorded voltage",centX+550,20);
}

void average(){
  noStroke();
  fill(255,255,255);
  rect(centX-490,15,ave*6*1E4,8);
}
...
void voltage(){
  ...
  for(int i=1;i<M;i++){
  fill(255,255,255);
  rect(centX+10+bar1[i-1]+10*i,15,bar1[i],8);
  }
}
void voltage2(){
  P = k%10;
  for (int s=1;s<=P;s++){ 
  fill(255,255,255);
  rect(centX+630+bar2[s-1]+10*s,15,bar2[s],8);
  }
}
